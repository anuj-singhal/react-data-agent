...
Enhanced with OpenAI embedding support
"""
...
class RAGSystem:
    """Complete RAG system for schema search and context building with OpenAI embeddings"""
    ...
    def _get_embedding(self, text: str, dimensions: int = None) -> List[float]:
        """
        Get embedding for a text using OpenAI API
        """
        try:
            # Set dimensions parameter if specified
            params = {
    ...
    def _get_embeddings_batch(self, texts: List[str], dimensions: int = None) -> List[List[float]]:
        """
        Get embeddings for multiple texts in a batch using OpenAI API
        """
        if not texts:
            return []
        ...
        try:
            ...
            # Fallback to one-by-one processing in case of errors
            embeddings = []
            for text in texts:
                try:
                    embedding = self._get_embedding(text, dimensions)
                    embeddings.append(embedding)
                except Exception as inner_e:
                    logger.error(f"Error in fallback embedding: {inner_e}")
                    # Add empty embedding as placeholder
                    embeddings.append([0.0] * (dimensions or self.config.EMBEDDING_DIMENSIONS))
            return embeddings
    ...
    def _update_collection(self, collection, texts, metadatas, ids):
        """Update ChromaDB collection using OpenAI embeddings"""
        if texts:
            try:
            ...
            # Get embeddings using OpenAI API (batch processing)
    ...
    def _index_query_history(self):
        ...
        if self.query_texts:
            ...
            # Get embeddings using OpenAI API (batch processing)
            self.query_embeddings = np.array(self._get_embeddings_batch(self.query_texts))
    ...
    def _find_similar_query_enhanced(self, query: str, threshold: float = 0.90) -> Optional[Dict[str, Any]]:
        ...
        """
        if not self.query_history or self.query_embeddings is None or len(self.query_embeddings) == 0:
            logger.info("No query history available for matching")
            return None
        
        print(f"\n{Fore.BLUE}🔍 Searching query history...{Style.RESET_ALL}")
        
        # Encode the input query using OpenAI API
        ...
        # Ensure both arrays are properly shaped for dot product
