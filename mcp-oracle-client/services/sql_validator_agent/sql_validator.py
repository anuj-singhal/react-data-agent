...
class SQLValidator:
    ...
    def __init__(self, schema_context, provider: str = "openai", api_key: Optional[str] = None, model: Optional[str] = None):
    ...
    async def _validate(self, prompt: str) -> Dict[str, Any]:
        """Call OpenAI API for validation"""
        try:
            response = self.llm.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert SQL validator specialized in different SQL Validation. Always respond with valid JSON."},
...
"""
    ...
    def _create_schema_validation_prompt(self, sql_query: str) -> str:
...
"""
    ...
    def _create_execution_validation_prompt(self, sql_query: str, execution_result: Dict) -> str:
...
"""
    ...
    def _create_correctness_validation_prompt(self, nl_query: str, sql_query: str) -> str:
...
"""
    ...
    async def validate_syntax(self, sql_query: str) -> ValidationResult:
        ...
        prompt = await self._create_syntax_validation_prompt(sql_query)
    ...
    def validate_schema(self, sql_query: str) -> ValidationResult:
        ...
        prompt = self._create_schema_validation_prompt(sql_query)
        response = self._validate(prompt)
    ...
    def validate_execution(self, sql_query: str) -> ValidationResult:
        ...
        prompt = self._create_execution_validation_prompt(sql_query, execution_result)
        response = self._validate(prompt)
    ...
    def validate_correctness(self, nl_query: str, sql_query: str) -> ValidationResult:
        ...
        prompt = self._create_correctness_validation_prompt(nl_query, sql_query)
        response = self._validate(prompt)
    ...
    async def validate_sql(self, nl_query: str, sql_query: str) -> OverallValidation:
